...
const Thumbnail = ({ event, eventType, setDisplay }) => {
...
=======
    ...
    const paymentObject = new window.Razorpay(options);
>>>>>>> 5b8e1ed9de2aa1ebf9375299031604331b5d1776
    paymentObject.open();
  }
...
<<<<<<< HEAD
        {(eventType === 'upcoming') && <div className="absolute right-0 bottom-2" onClick={()=>setDisplay(true)}>
          <div className="rounded-2xl cursor-pointer duration-200 delay-75 transition hover:bg-red-600 hover:text-white bg-white text-black p-1 mr-2 font-semibold">Book Now</div>
        </div>}
      </div>

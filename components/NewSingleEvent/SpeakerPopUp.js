...
const SpeakerPopUp = ({displaySpeaker,eventDetails}) =>{
    ...
    for (let i = 0; i < speakerDetails.resources.length; i++) {
        ...
        LinksList.push(
            <a key={i} href={speakerDetails.resources[i]} target="_blank" className="hover:text-gray-400">{linkToPush}</a>
        );
    }
    ...
    return(
        ...
        <div id="backdrop" onClick={onClickHandler} className="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center z-30 ">
            <div className="flex flex-col absolute w-2/3 md:w-3/5 bg-gray-600 text-black rounded-md p-5 border-[3px] border-gray-700">

                {/* Bio Section */}
                <div className="flex flex-col sm:flex-row mb-4">
                    <div className="flex item-center h-full justify-center ">
                        <img
                        className="h-[7rem] w-[7rem] m-2 items-center justify-center rounded-md border-2 border-gray-700"
                        src={speakerDetails.imageUrl}
                        />
                    </div>
                    <div className="m-2 flex flex-col md:h-2/3">
                        <div className="font-bold text-lg text-center sm:text-left ">{speakerDetails.name}</div>
                        <div className="text-sm font-light text-center sm:text-left">{speakerDetails.bio}</div>
                    </div>
                </div>
